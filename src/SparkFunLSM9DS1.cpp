...
bool LSM9DS1::reset() {
  xgWriteByte(CTRL_REG8, xgReadByte(CTRL_REG8) | (1 << 0));
  mWriteByte(CTRL_REG2_M, mReadByte(CTRL_REG2_M) | (1 << 2));
  ...
  uint8_t swReset;
	...
	do {
		uint8_t r1 = xgReadByte(CTRL_REG8);
		uint8_t r2 = mReadByte(CTRL_REG2_M);
	...
	} while (softReset & swReset);

  return (softReset & swReset) == 0;
}
